...
\begin{abstract}
We construct directional wavelet systems that will enable building efficient signal representation schemes with good direction selectivity. In particular, we focus on wavelet bases on a dyadic quincunx lattice; dilated quincunx downsampling is used to construct orthonormal and bi-orthogonal bases and standard dyadic downsampling for low-redundancy frames. We show that the supports of both orthonormal and bi-orthogonal wavelets in our framework are discontinuous in the frequency domain. However, this irregularity constraint can be avoided in frames, even with redundancy factor less than 2. 
\end{abstract}
